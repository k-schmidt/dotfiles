...
If context is insufficient to produce a meaningful model, say what's missing and suggest running `$grill-with-docs` or `$to-prd` first. Do NOT interrogate — this skill synthesizes, it doesn't interview.
...
Use the project's domain vocabulary from the applicable `docs/CONTEXT.md`, resolving it through `docs/CONTEXT-MAP.md` when present. If the PRD introduced terms in its Technical Decisions, carry those forward into entity and attribute names.

### 5. Write the document

Follow a repository-specific documentation layout when its instructions declare one. Otherwise use these defaults:

1. **Safety check:** Check if `docs/DATA-MODEL-*.md` files already exist. If they do, list them and ask whether to overwrite or use a different name.
2. Write to `docs/DATA-MODEL-{feature-name}.md`. Create the `docs/` directory if needed.
...
- [DATA-MODEL-{feature-name}](../docs/DATA-MODEL-{feature-name}.md)
```
