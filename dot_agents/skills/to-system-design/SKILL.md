...
If context is insufficient to produce a meaningful design, say what's missing and suggest running `$grill-with-docs` or `$to-prd` first. Do NOT interrogate — this skill synthesizes, it doesn't interview.
...
Use the project's domain vocabulary from the applicable `docs/CONTEXT.md`, resolving it through `docs/CONTEXT-MAP.md` when present. If the PRD introduced terms in its Technical Decisions, carry those forward into diagram labels and section headings.

### 5. Write the document

Follow a repository-specific documentation layout when its instructions declare one. Otherwise use these defaults:

1. **Safety check:** Check if `docs/SYSTEM-DESIGN-*.md` files already exist. If they do, list them and ask whether to overwrite or use a different name.
2. Write to `docs/SYSTEM-DESIGN-{feature-name}.md`. Create the `docs/` directory if needed.
3. Cross-link to the PRD: include `**PRD:** [PRD-{feature-name}](../specs/PRD-{feature-name}.md)` in the header.
...
- [SYSTEM-DESIGN-{feature-name}](../docs/SYSTEM-DESIGN-{feature-name}.md)
```
...
- "Run `$to-tasks` to break this into implementation tasks"
...
- Use domain terms from the applicable `docs/CONTEXT.md` as node labels, not generic names
