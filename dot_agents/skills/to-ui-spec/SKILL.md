...
If context is insufficient to produce a meaningful spec, say what's missing and suggest running `$grill-with-docs` or `$to-prd` first. Do NOT interrogate — this skill synthesizes, it doesn't interview.
...
Use the project's domain vocabulary from the applicable `docs/CONTEXT.md`, resolving it through `docs/CONTEXT-MAP.md` when present. If the PRD introduced terms, carry those forward into screen names and content descriptions.

### 5. Write the document

Follow a repository-specific documentation layout when its instructions declare one. Otherwise use these defaults:

1. **Safety check:** Check if `docs/UI-SPEC-*.md` files already exist. If they do, list them and ask whether to overwrite or use a different name.
2. Write to `docs/UI-SPEC-{feature-name}.md`. Create the `docs/` directory if needed.
...
- [UI-SPEC-{feature-name}](../docs/UI-SPEC-{feature-name}.md)
```
